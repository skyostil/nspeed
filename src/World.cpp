...
void Object::render(View *view)
{
	...
	while(vc--)
	{
		*vt = *v;
		...
		vt->pos = view->camera.transform(vt->pos);
	...
	while(fc--)
	{
		...
		normal = view->camera.transformDirection(normal);
		
		if (normal.dot(*eye) < 0)
		{
			view->beginPolygon();
			view->rasterizer->setTexture(f->texture);
			view->rasterizer->setColor(f->color);
			for(vc=0; vc<f->vertexCount; vc++)
			{
				v = &transformedVertex[f->vertex[vc]];
	//			printf("%d, %d, %d\n", v->pos.x>>FP, v->pos.y>>FP, v->pos.z>>FP);
				view->addTransformedVertex(*v);
			}
			view->endPolygon();
		}
		f++;
