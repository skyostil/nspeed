...
public:
	...
	View		*view;
	Game::Surface	*texture, *texture2, *texture3;
	Game::Surface	*textureTileList[256];
	Object		*object;
	Land		*track, *ground, *sky;
	scalar		t;
	...
	{
#ifdef LAND
		land.normal.set(0,0,1);
		land.dist = 0;
		cam.origin.origin.set(1,0,1);
		cam.target.origin.set(0,0,0);
		cam.update();
#endif
	}
...
*/					
	...
	//			*pixels++ = rand();
		...
		textureTileList[1] = framework->loadImage("../../data/test.png", &screen->format);
		textureTileList[2] = framework->loadImage("../../data/test2.png", &screen->format);
#endif		
		
		Game::PixelFormat pf(8);
//		texture3 = new Game::Surface(&pf, 256, 256);
		track = new Land(framework->loadImage("../../data/track.png", &pf));
//		texture3->clear(0);

		ground = new Land(framework->loadImage("../../data/ground.png", &screen->format), 0, FPInt(4), FPInt(0), FPInt(2), FPInt(256));
		sky = new Land(framework->loadImage("../../data/sky.png", &screen->format), 0, FPInt(6), FPInt(1), FPInt(0), FPInt(64));
...
#if 0
#ifdef LAND
		scalar t = FPInt(framework->getTickCount()) / 100;
		cam.origin.origin.set(FPCos(t)+t,FPSin(t),1+FPSin(t/4)/4);
		cam.target.origin.set(t,0,1);
		cam.update();

		land.render(screen, cam);
#else
#endif
#endif
	}
...
*/		
	...
	void handleEvent(Game::Event* event)
	{
		...
		{
			mixer->playSample(sample2, (rand()%sample2->rate) + sample2->rate/2);
